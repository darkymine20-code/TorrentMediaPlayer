...
extension TorrentService: SessionDelegate {
    func torrentManager(_ manager: Session, didAddTorrent torrent: TorrentHandle) {
        ...
        // Automatically set sequential download mode & piece priorities for media streaming
        AutoPlayService.shared.configureAutoSequential(for: torrent)

        // Start peer reconnect monitoring if needed
        PeerReconnectService.shared.performPeerReconnectionCheck()
...
private extension TorrentService {
    func setup() {
        ...
        // Initialize Torrent Media Player automation services
        _ = AutoPlayService.shared
        _ = PeerReconnectService.shared
